...
class Person :
    ...
    # : info() : 추가로 저장된 정보를 출력하는 매서드
    def info(self) : 
        print('{}, {}, {}'.format(self.name,self.phone, self.address))

#* AddressBook 클래스 생성
# : 주소록 프로젝트의 모든 기능을 구현하는 클래스
# : Person 클래스의 객체를 여러 개 저장할 수 있는 address_list 리스트를 포함

class AddressBook :
    
    # 생성자
    # : csv 파일을 읽어서 address_list 리스트에 저장
    # : 리스트 생성 후 csv 파일을 읽어오는 file_reader() 매서드 호출
    def __init__(self) :
        self.address_list = []
        self.file_reader()
        
    # csv 파일 읽기
    # : csv 파일의 정보를 address_list에 저장하는 매서드
    def file_reader(self) :
        try : # csv 파일이 없으면 예외 발생
            ...
            while True :
                buffer = file.readline() # csv 파일을 한 줄씩 읽어 들이는 매서드
            if not buffer :
                break
            # split 매서드를 사용하여 이름, 전화번호, 주소를 분리
            name = buffer.split(',')[0]
    ...
    # csv 파일 생성
    # : address_list의 모든 데이터를 사용하여 csv 파일을 생성하는 매서드
    ...
    # 프로그램 실행
    # : 사용자로부터 입력받은 값으로 작업할 해당 매서드를 호출
    def run(self) :
        while True :
            # menu() 매서드는 정적 매서드 - 클래스 명으로 호출
            ...
            elif choice == 4 : self.search() # 전체 주소록 출력
            elif choice == 5 : self.print_all()
            else : print('없는 번호입니다. 확인 후 다시 입력하세요.')
            
    # insert()
    # : 사용자로부터 새로 등록할 이름, 전화번호, 주소를 입력받고 Person 객체를 생성하여 해당 객체를 address_list에 저장
    def inser(self) :
        print('===  신규 주소록 생성 ===')
