...
# 5. zip() 함수
 # : 여러 개의 순회 가능한(iterable) 값을 받아 각 객체의 동일한 인덱스에 있는 요소들기리 짝지어 튜플로 변환
...
for i in result :
    print(i)
  # 결괏값 : 각각의 순회 가능한 값의 길이가 다른 경우 동일한 인덱스 번호의 값만을 추출(가장 짧은 길이의 값을 기준으로 튜플을 반환)
  # (1, 'one')
  # (2, 'two')